...
h1_price <- cor.test(dane_imputed$centreDistance, dane_imputed$price, method = "spearman")
h1_pm2   <- cor.test(dane_imputed$centreDistance, dane_imputed$price_m2, method = "spearman")

h1_price
h1_pm2

h4_price <- cor.test(dane_imputed$squareMeters, dane_imputed$price, method = "spearman")
h4_pm2   <- cor.test(dane_imputed$squareMeters, dane_imputed$price_m2, method = "spearman")
...
wilcox.test(price_m2 ~ hasBalcony,
            data = dane_imputed)
wilcox.test(price_m2 ~ hasParkingSpace,
            data = dane_imputed)
wilcox.test(price_m2 ~ hasElevator,
            data = dane_imputed)
wilcox.test(price_m2 ~ hasSecurity,
            data = dane_imputed)
wilcox.test(price_m2 ~ hasStorageRoom,
            data = dane_imputed)
