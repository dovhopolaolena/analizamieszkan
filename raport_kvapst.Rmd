...
gg_miss_var(dane_clean)
vis_miss(dane_clean)
gg_miss_upset(dane_clean)
missing_pattern(dane_clean)
aggr_plot <- VIM::aggr(dane_clean) 
gg_miss_fct(x=dane_clean,fct=collegeDistance)
gg_miss_fct(x=dane_clean,fct=type)
gg_miss_fct(x=dane_clean,fct=floor)
gg_miss_fct(x=dane_clean,fct=hasElevator)
```
...
```{r}
dane_clean$hasElevator <- as.factor(dane_clean$hasElevator)
dane_clean$type <- as.factor(dane_clean$type)
